...
#devtools::install_local("UNZCubeDataTools_0.1.0.tar.gz")

# feature flags

feature.recent_cohorts <- TRUE
feature.disable_dataset2 <- FALSE


feature.aggregation_notes <- FALSE
feature.suppression_notes <- FALSE
# end of flags

featureControl <- function(flag, result) {
  if (flag) {
    return(result)
  } else {
    return(NULL)
...
dir.src <- "scripts"
dir.pages <- "scripts/pages"


source(file.path(dir.src, "branding functions.R"))
source(file.path(dir.src, "IndicatorChartModule.R"))
source(file.path(dir.src, "IndicatorD3ChartModule.R"))
source(file.path(dir.src, "IndicatorD3MultilineModule.R"))


source(file.path("./inst/UNZCubeDataTools/shinyappload.R"))
source(file.path(dir.src, "utility functions.R"))

source(file.path(dir.pages, "monthlyIndicatorPage.R"))
source(file.path(dir.pages, "disclaimerPage.R"))
source(file.path(dir.pages, "cohortComparisionPage.R"))
source(file.path(dir.pages, "multiIndicatorPage.R"))
source(file.path(dir.pages, "landingPage.R"))

#if(!"UNZCubeDataTools" %in% installed.packages()) {
#  devtools::install_local("UNZCubeDataTools_0.1.0.tar.gz")
#}
